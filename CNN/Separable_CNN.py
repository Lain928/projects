'''
    实现深度可分离卷积
    将除了输入层的 Conv2D 其余的全部改为 SeparableConv2D
'''
import os
import numpy as np
...
def Creat_CNN_Model():
    ...
    model.add(tf.keras.layers.Conv2D(filters=32,kernel_size=3,
                                     padding='same',
                                     input_shape=(28,28,1)))
    model.add(tf.keras.layers.SeparableConv2D(filters=32,kernel_size=3,
                                     padding='same'))

    # 池化层
    model.add(tf.keras.layers.MaxPool2D(pool_size=2))
    # 卷积层
    model.add(tf.keras.layers.SeparableConv2D(filters=64,kernel_size=3,
                                     padding='same'))
    # 池化层
    model.add(tf.keras.layers.MaxPool2D(pool_size=2))
    # 卷积层
    model.add(tf.keras.layers.SeparableConv2D(filters=128,kernel_size=3,
                                     padding='same'))
    # 池化层
    model.add(tf.keras.layers.MaxPool2D(pool_size=2))
